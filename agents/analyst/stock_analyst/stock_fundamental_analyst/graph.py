...
流程：
- fetch_only: fetch -> analysis -> END
- full: fetch -> analysis -> company_basic_insight -> [5个map并行] -> reduce -> END
"""
...
def create_stock_fundamental_fetch_only_graph() -> Any:
    ...
    builder.add_edge(START, "stock_fundamental_fetch")
...
def create_stock_fundamental_analyst_graph(llm=None) -> Any:
    ...
    builder.add_edge("fundamental_reduce", END)
    return builder.compile()
