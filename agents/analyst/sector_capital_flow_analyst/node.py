...
def create_sector_capital_flow_fetch_node():
    ...
    def sector_capital_flow_fetch_node(
        ...
        # 申万行业日线（用于估算行业资金活跃度）
        try:
            from dataflow.industry_data import fetch_sw_daily_range
        except Exception as e:  # pragma: no cover
            logger.warning("无法导入 fetch_sw_daily_range: %s", e)
            fetch_sw_daily_range = None  # type: ignore[assignment]
        ...
        sw_df = None
        if fetch_sw_daily_range is not None:
            try:
                # 只取必要字段，避免数据量过大
                sw_df = fetch_sw_daily_range(
                    start_date=start_date,
                    end_date=end_date,
                    fields="ts_code,name,trade_date,pct_change,amount",
                )
            except Exception as e:
                logger.warning(
                    "fetch_sw_daily_range 失败: start=%s end=%s error=%s",
                    start_date,
                    end_date,
                    e,
                )
        ...
        return {
            ...
            "sw_industry_daily_data": to_serializable(sw_df),
...
def create_sector_capital_flow_analysis_node():
    ...
    - 申万行业：基于 amount 与 pct_change 构造简单资金强度指标，给出同样的
      流入/流出前 10 名（sector_capital_flow_top.sw_industry）。
    全程不依赖 LLM，仅做规则与排序。
    """
    ...
    def sector_capital_flow_analysis_node(
        ...
        sw_records = state.get("sw_industry_daily_data") or []
        if not ths_records and not sw_records:
        ...
        if not end_date_str:
            dates: List[str] = []
            for row in ths_records or sw_records:
        ...
        # ------------------------------
        # 2）申万行业资金强弱（基于 sw_daily）
        # ------------------------------
        sw_per_code: Dict[str, Dict[str, Any]] = {}
        for row in sw_records:
            try:
                ts_code = row.get("ts_code")
                name = row.get("name", ts_code)
            ...
            if not ts_code:
                continue
            info = sw_per_code.setdefault(ts_code, {"name": name, "rows": []})
            # 简单构造一个资金强度指标：amount * pct_change（单位无量纲，只用于排序）
            flow_strength = amount * pct_change
            info["rows"].append(
                {
                    "trade_dt": trade_dt,
        ...
        sw_windows: Dict[str, Dict[str, Dict[str, Any]]] = {}
        for ts_code, info in sw_per_code.items():
            rows = info.get("rows") or []
            if not rows:
                continue

            win_map: Dict[str, Dict[str, Any]] = {}
            for w in _WINDOWS:
                # 按日期从近到远排序，窗口内不足 w 天时向前补足最近的 w 个交易日
                rows_sorted = sorted(rows, key=lambda r: r["trade_dt"], reverse=True)
                selected = rows_sorted[:w]
                if not selected:
                    continue
                days = len(selected)
                flow_sum = sum(r["flow_strength"] for r in selected)
                amount_sum = sum(r["amount"] for r in selected)
                avg_flow = flow_sum / max(days, 1)

                win_map[f"{w}d"] = {
                    "ts_code": ts_code,
                    "name": info.get("name", ts_code),
                    "days": days,
                    "flow_strength_sum": round(flow_sum, 2),
                    "flow_strength_avg": round(avg_flow, 2),
                    "amount_sum": round(amount_sum, 2),
                }

            if win_map:
                sw_windows[ts_code] = win_map

        sw_top_by_window: Dict[str, Dict[str, List[Dict[str, Any]]]] = {}
        for w in _WINDOWS:
            key = f"{w}d"
            entries = [
                win_map[key]
                for _, win_map in sw_windows.items()
                if key in win_map
            ]
            if not entries:
                continue
            inflow_sorted = sorted(entries, key=lambda x: x["flow_strength_sum"], reverse=True)
            outflow_sorted = sorted(entries, key=lambda x: x["flow_strength_sum"])
...
_MAP_SW_FIELDS = """【申万 sw_industry 每条字段】
- ts_code、name、days、window、rank_inflow、rank_outflow：同上
- flow_strength_sum：窗口内「成交额×涨跌幅」汇总，放量涨为正；flow_strength_avg、amount_sum
键 "1d"/"5d"/"10d"/"20d" 与 top_inflow、top_outflow 同上。"""


def _run_map_for_source(
    llm,
    source: str,
    state: Dict,
    ...
    """
    对单个数据源执行 Map 解读。
    并行时返回 {"sector_flow_map_results": [{"source": source, **parsed}]} 供 Annotated+operator.add 合并。
    """
    field_doc = _MAP_THS_FIELDS if source == "ths_concept" else _MAP_SW_FIELDS
    source_name = "同花顺概念/板块" if source == "ths_concept" else "申万行业"

    top = state.get("sector_capital_flow_top") or {}
    data = top.get(source) or {}
    if not data:
        one = {"source": source, **_SECTOR_FLOW_MAP_DEFAULT, "summary": f"无{source_name}数据"}
        return {"sector_flow_map_results": [one]}

    from langchain_core.prompts import ChatPromptTemplate

    data_str = json.dumps(data, ensure_ascii=False, indent=2)
    system_msg = f"""你是一位板块资金流分析师。下方是「{source_name}」多窗口资金流 Top 的 JSON，请根据字段含义做简短解读。

{field_doc}

解读要点（写入 highlights）：主线方向、资金认可与持续性（flow_days_ratio、flow_trend_label）、短中长期是否一致。再列出 hot_candidates（资金持续流入/强度居前的名称）、risk_candidates（持续流出或需警惕的名称）。

返回严格 JSON，只输出 JSON：
{{{{
  "summary": "2-3 句话概括该数据源资金流向",
  "highlights": ["要点1", "要点2"],
  "hot_candidates": ["板块/行业名称"],
  "risk_candidates": ["板块/行业名称"]
}}}}
无明确规律时 summary 可写「无明显方向」。"""

    human_msg = """【{source_name} 数据】
{data}
...
请分析并返回上述 JSON。"""
    ...
    logger.info("正在处理 板块资金流 LLM Map (%s)", source)
    chain = prompt | llm
    raw = chain.invoke(
        {"data": data_str, "source_name": source_name},
        config={**(config or {}), "run_name": f"板块资金流Map_{source}"},
    )
    parsed = extract_json_text(raw)
    for k, v in _SECTOR_FLOW_MAP_DEFAULT.items():
        parsed.setdefault(k, v)
    one = {"source": source, **parsed}
    return {"sector_flow_map_results": [one]}


def create_sector_capital_flow_llm_map_node(llm, source: Optional[str] = None):
    """
    构建板块资金流 Map 节点。
    - 若传入 source（"ths_concept" 或 "sw_industry"），则固定处理该数据源。
    - 若 source 为 None，则从 state["_map_source"] 读取数据源（用于 Send 并行时由 payload 指定）。
    """
    def _map_node(state: Dict, config: Optional[RunnableConfig] = None) -> Dict:
        src = source if source is not None else (state.get("_map_source") or "ths_concept")
        return _run_map_for_source(llm, src, state, config)
    return _map_node


# ---------------------------------------------------------------------------
# sector_capital_flow_llm_reduce（合并两路 Map 输出为最终报告）
# ---------------------------------------------------------------------------
...
def create_sector_capital_flow_llm_reduce_node(llm):
    """
    读取 sector_flow_map_ths 与 sector_flow_map_sw，合并为最终 sector_capital_flow_insight。
    """

    def _reduce_node(state: Dict, config: Optional[RunnableConfig] = None) -> Dict:
        # 从并行 Map 合并后的 list 里按 source 拆成 ths / sw
        results = state.get("sector_flow_map_results") or []
        map_ths = _SECTOR_FLOW_MAP_DEFAULT.copy()
        map_sw = _SECTOR_FLOW_MAP_DEFAULT.copy()
        for item in results:
            src = item.get("source")
            row = {k: v for k, v in item.items() if k != "source"}
            if src == "ths_concept":
                map_ths = row
            elif src == "sw_industry":
                map_sw = row

        # 若两边都无有效 summary，直接给默认结论
        if not (map_ths.get("summary") or map_sw.get("summary")):
            return {
        ...
        input_str = f"""【同花顺概念/板块解读】
summary: {map_ths.get('summary', '')}
highlights: {map_ths.get('highlights', [])}
hot_candidates: {map_ths.get('hot_candidates', [])}
risk_candidates: {map_ths.get('risk_candidates', [])}

【申万行业解读】
summary: {map_sw.get('summary', '')}
highlights: {map_sw.get('highlights', [])}
hot_candidates: {map_sw.get('hot_candidates', [])}
risk_candidates: {map_sw.get('risk_candidates', [])}"""

        system_msg = """你是一位板块资金流分析师。上面是同花顺概念/板块与申万行业的两段分别解读（含 summary、highlights、hot_candidates、risk_candidates）。请合并为一份最终报告：综合两源结论、判断整体市场资金面倾向、归纳热点与风险板块。

返回严格 JSON，只输出 JSON：
{{
  "summary": "概括整体资金流向与结构",
  "conclusion": "结论（可含操作或观望建议）",
  "highlights": ["合并后的要点，含主线/持续性/拐点等，每条一句"],
  "market_bias": "bullish | neutral | bearish",
  "hot_sectors": ["综合两源后的热点板块/行业名称列表"],
  "risk_sectors": ["综合两源后的风险/弱势板块/行业名称列表"]
}}
market_bias 表示整体资金面偏多/中性/偏空。无明显方向时 market_bias 填 neutral。"""

        human_msg = """{input_text}
...
请合并为上述 JSON。"""
        ...
        logger.info("正在处理 板块资金流 LLM Reduce")
        chain = prompt | llm
        raw = chain.invoke(
            {"input_text": input_str},
            config={**(config or {}), "run_name": "板块资金流Reduce"},
        )
