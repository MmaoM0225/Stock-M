...
def create_sector_trend_fetch_node():
    def _fetch(state: Dict, config: Optional[RunnableConfig] = None) -> Dict:
        ...
        try:
            from dataflow.industry_data import fetch_sw_daily_range, fetch_ths_daily_range, fetch_ths_index
        except Exception as e:
            logger.warning("导入行业数据接口失败: %s", e)
            return {"sw_daily_data": [], "ths_daily_data": [], "ths_code_to_name": {}, "sector_trend_meta": {"start_date": start_date, "end_date": end_date}}

        sw_df = fetch_sw_daily_range(
            start_date=start_date,
            end_date=end_date,
            fields="ts_code,name,trade_date,open,high,low,close,pct_change,vol",
        )
        ...
        try:
            ...
            if idx is not None and not idx.empty and "ts_code" in idx.columns and "name" in idx.columns:
                ths_map = dict(zip(idx["ts_code"], idx["name"]))
        except Exception:
            pass

        return {
            "sw_daily_data": to_serializable(sw_df),
...
def create_sector_trend_llm_map_node(llm, source: Optional[str] = None):
    def _map_node(state: Dict, config: Optional[RunnableConfig] = None) -> Dict:
        ...
        payload = _build_trend_map_payload(state, src)
        source_name = payload.get("source_name") or src
        has_content = bool(
        ...
        if not has_content:
            one = {
                "source": src,
                **_SECTOR_TREND_MAP_DEFAULT,
                "summary": f"无{source_name}榜单数据",
            }
            return {"sector_trend_map_results": [one]}

        from langchain_core.prompts import ChatPromptTemplate

        payload_str = json.dumps(payload, ensure_ascii=False, indent=2)
        system_msg = f"""你是一位行业趋势分析师。下面是「{source_name}」的轻量榜单摘要 JSON，请根据字段做简洁解读。
...
返回严格 JSON，只输出 JSON：
{{{{
  "summary": "2-3 句话总结当前行业/概念榜单结构",
  "highlights": ["要点1", "要点2", "要点3"],
  "main_trends": ["强势主线名称"],
  "reversal_watchlist": ["反转修复方向名称"],
  "top_risk_watchlist": ["高位风险方向名称"]
}}}}
若无明显规律，可保持列表为空，summary 写“无明显主线”。"""
        human_msg = """【榜单轻量摘要】
{payload}
...
请分析并返回上述 JSON。"""
        ...
        logger.info("正在处理 行业趋势 LLM Map (%s)", src)
        chain = prompt | llm
        raw = chain.invoke(
            {"payload": payload_str},
            config={**(config or {}), "run_name": f"行业趋势Map_{src}"},
        )
        parsed = extract_json_text(raw)
        for k, v in _SECTOR_TREND_MAP_DEFAULT.items():
            parsed.setdefault(k, v)
        return {"sector_trend_map_results": [{"source": src, **parsed}]}

    return _map_node


def create_sector_trend_llm_reduce_node(llm):
    def _reduce_node(state: Dict, config: Optional[RunnableConfig] = None) -> Dict:
        results = state.get("sector_trend_map_results") or []
        map_ths = _SECTOR_TREND_MAP_DEFAULT.copy()
        map_sw = _SECTOR_TREND_MAP_DEFAULT.copy()
        for item in results:
            src = item.get("source")
...
main_trends: {map_ths.get('main_trends', [])}
reversal_watchlist: {map_ths.get('reversal_watchlist', [])}
top_risk_watchlist: {map_ths.get('top_risk_watchlist', [])}

【申万行业解读】
summary: {map_sw.get('summary', '')}
highlights: {map_sw.get('highlights', [])}
main_trends: {map_sw.get('main_trends', [])}
reversal_watchlist: {map_sw.get('reversal_watchlist', [])}
top_risk_watchlist: {map_sw.get('top_risk_watchlist', [])}"""

        system_msg = """你是一位行业趋势分析师。上面是同花顺概念/板块与申万行业的两段榜单解读。请把两源结果合并成一份最终结论，重点回答：
1. 当前市场更像趋势延续、轮动切换、修复回暖还是风险释放
2. 哪些方向是主线
3. 哪些方向值得作为反转修复观察对象
4. 哪些方向要警惕高位转弱
...
请合并为上述 JSON。"""
        ...
        logger.info("正在处理 行业趋势 LLM Reduce")
        chain = prompt | llm
        raw = chain.invoke(
            {"input_text": input_str},
            config={**(config or {}), "run_name": "行业趋势Reduce"},
        )
